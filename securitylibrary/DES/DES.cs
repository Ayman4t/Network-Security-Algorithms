...
namespace SecurityLibrary.DES
{
    ...
    public class DES : CryptographicTechnique
    {
        ...
        #endregion
        #region Functions
        public string Permutaion(int[,] pTable, string input, int row, int column)
        {
            string result = "";
            for (int i = 0; i < row; i++)
            {
        ...
        // Overriding the Encrypt method from the base class
        // Overriding the Encrypt method from the base class
        public override string Encrypt(string plainText, string key)
        {
            // Convert the hexadecimal key to binary and pad to 64 bits
            string key_64 = Convert.ToString(Convert.ToInt64(key, 16), 2).PadLeft(64, '0');
            List<string> key1_shifted = new List<string>();
            List<string> key2_shifted = new List<string>();
            // Perform permutation on the key to obtain a 56-bit key
            string key_56 = Permutaion(PC_1, key_64, 8, 7);
            string key1_28 = key_56.Substring(0, 28); // Split the key into two halves
            string key2_28 = key_56.Substring(28, 28);
            // Perform circular shift left operations on the two halves of the key
            CircularShiftLeft(key1_shifted, key2_shifted, key2_28, key1_28);
            List<string> keys_56 = new List<string>();
            // Combine the shifted halves to form 16 round keys of 56 bits each
            for (int i = 0; i < key2_shifted.Count; i++)
            {
                keys_56.Add(key1_shifted[i] + key2_shifted[i]);
            }
            // Generate 16 round keys of 48 bits each by performing another permutation
            List<string> keys_16 = Add_round_keys(keys_56);
            // Convert the hexadecimal plaintext to binary and pad to 64 bits
            ...
            List<string> Left_plain = new List<string>();
            List<string> Right_plain = new List<string>();
            string l_plain = plain_56.Substring(0, 32); // Split the plaintext into two halves
            ...
            for (int i = 0; i < 16; i++)
            {
                Left_plain.Add(r_plain); // Set left plain to previous right plain
                string XOR = "";
                Separated_plain.Clear();
                string plain_48 = Permutaion(EB, r_plain, 8, 6); // Perform expansion permutation on right plain
                                                                 // XOR operation between the expanded plaintext and round key
                for (int j = 0; j < plain_48.Length; j++)
                {
                    XOR += (keys_16[i][j] ^ plain_48[j]).ToString();
                }
                // Separate the XOR result into 6-bit chunks
                for (int w = 0; w < XOR.Length; w += 6)
                {
                    string tmp = "";
                    for (int o = w; o < w + 6; o++)
                ...
                // Apply S-box substitution to the separated plaintext
                string sbox_result = SBOX(Separated_plain);
                // Perform permutation on the S-box output
                string last_permutation = Permutaion(P, sbox_result, 8, 4);
                XOR = "";
                // XOR operation between the permuted S-box output and left plain
                for (int k = 0; k < last_permutation.Length; k++)
                {
                    XOR += (last_permutation[k] ^ l_plain[k]).ToString();
                }
                l_plain = r_plain; // Update left plain
                r_plain = XOR; // Update right plain
                Right_plain.Add(r_plain);
            }
            // Combine the final left and right plains
            string result_16 = Right_plain[16] + Left_plain[16];
            // Perform final permutation (inverse of initial permutation) to get ciphertext
            string ciphertext = Permutaion(IP_1, result_16, 8, 8);
            // Convert the binary ciphertext to hexadecimal
            string ct = Convert.ToInt64(ciphertext, 2).ToString("X");
            return "0x" + ct; // Return the hexadecimal ciphertext
        }
